...
@Composable
fun MainGreenButton(onClick: () -> Unit, text: String, isEnabled: Boolean = true, isLoading: Boolean = false, modifier: Modifier) {
    if(!isLoading){
        val colors = if(isEnabled){
            ButtonDefaults.buttonColors(
                containerColor = MainGreenLight,
        ...
        Button(
            ...
            Text(style = bottomSheetButtonStyle(), text = text, fontWeight = Bold)
        }
    }
    else{
...
@Composable
fun PreviewMainGreenButton() {
    MainGreenButton(text = "SomethingText", isEnabled = true, onClick = {}, modifier = Modifier.fillMaxWidth())
}